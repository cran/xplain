...
getLanguage <- function() {
  # Internal function that tries to identify the language of the user's work environment by performing an illegal operation and
  ...
  if(lang == "") {
    lang <- "EN"
    # Perform an illegal operation and capture the error message
    res <- tryCatch( { "abc"/1 }, error = function(err) { return(err) })
    # Determine the language based on the error message
    if(regexpr("nicht-numerisches Argument", res$message) != -1) lang <- "DE"
    if(regexpr("argument non", res$message) != -1) lang <- "FR"
    if(regexpr("argomento non numerico", res$message) != -1) lang <- "IT"
    if(regexpr(".* para operador binario.*", res$message) != -1) lang <- "ES"
    if(regexpr("argument nieliczbowy przekazany", res$message) != -1) lang <- "PL"
    if(regexpr("sayisal olmayan", res$message) != -1) lang <- "TR"
  }
  return(toupper(lang))
}
